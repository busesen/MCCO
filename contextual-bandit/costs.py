...
def cost_ymean(u, params, shift=None, Test=False):
    ...
    # Group 1 values (c1 == 0)
    mu_a1_g1 = p_g1[0] + p_g1[1] * c5
    mu_a2_g1 = p_g1[2] + p_g1[3] * c5
    
    # Group 2 values (c1 != 0)
    mu_a1_g2 = p_g2[0] + p_g2[1] * c5
    mu_a2_g2 = p_g2[2] + p_g2[3] * c5
    
    # Combine
    mu_a1 = mu_a1_g1 * (1.0 - c1_bin) + mu_a1_g2 * c1_bin
    mu_a2 = mu_a2_g1 * (1.0 - c1_bin) + mu_a2_g2 * c1_bin
    ...
    TAIL_AMP = torch.tensor(12.0, dtype=mu_a1.dtype, device=mu_a1.device)

    c5_center = torch.tensor(2.5, dtype=mu_a1.dtype, device=mu_a1.device)
    c5_scale = torch.tensor(2.5, dtype=mu_a1.dtype, device=mu_a1.device)
    ...
    tail_mask_f = tail_mask.to(dtype=mu_a1.dtype)
    tail_bump = TAIL_AMP * bump_shape * tail_mask_f

    mu_a1 = mu_a1 + tail_bump
    mu_a2 = mu_a2 + tail_bump
    ...
    if Test and shift is not None:
        shift_t = to_t(shift)
        mu_a1 = mu_a1 + shift_t[0]
        mu_a2 = mu_a2 + shift_t[1]

    # ---------------------------------------------------------
    # Return
    # ---------------------------------------------------------
    res = torch.stack([mu_a1, mu_a2], dim=-1)
